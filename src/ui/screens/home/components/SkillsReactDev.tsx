...
import { ModalPortal, Icon } from "@/ui/components";
...
const Article: React.FC<ArticleProps> = ({
  ...
  return (
    <ModalPortal modalTitle={title} actionButtonText={title}>
      <article className={`${bgColor} ${bgBorder} flex flex-col rounded-xl space-y-4 p-2 md:p-4`}>
        <header>
          <figure
            className={`${bgListColor} ${bgBorder} float-right flex justify-center items-center rounded-full my-2 ml-2 p-3 overflow-hidden`}
          >
...
const SkillsReactDev: React.FC = () => {
  ...
  return (
    <section
      ref={containerRef}
      className="overflow-x-auto flex flex-row-reverse gap-4"
    // className="snap-x snap-mandatory scroll-smooth"
    >
      {skillsDevData.frameworks.reverse().map((article, index) => (
        <div
          key={index}
          data-article
